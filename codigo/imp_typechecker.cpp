...
ImpType ImpTypeChecker::visit(CondExp* e) {
    ...
    ImpType ttrue = e->etrue->accept(this);
    ImpType tfalse = e->efalse->accept(this);

    if (tcond != TBOOL) {
        cout << "Tipo incorrecto en condicional" << endl;
        exit(0);
    }
    if (ttrue != tfalse) {
        cout << "Tipos en condicional no coinciden" << endl;
        exit(0);
    }
    return ttrue;
}
